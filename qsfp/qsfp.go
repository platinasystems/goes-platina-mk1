...
var qsfpVpageByKey map[string]uint8
			...
			for i := 0; i < 16; i++ {
				if (1<<uint(i))&(buffPresent[j]^present[j]) != 0 {
					...
					if ((1 << uint(i)) & (buffPresent[j] ^ 0xffff)) != 0 {
						...
						v := qsfpVdev[i+j*16].Compliance()
						var portConfig string

						//identify copper vs optic and set media and speed
						media, err := redis.Hget(redis.DefaultHash, "vnet."+qsfpIfnameOf(lp, (1+porto))+".media")
						if err != nil {
							log.Print("qsfp hget error:", err)
						}
						speed, err := redis.Hget(redis.DefaultHash, "vnet."+qsfpIfnameOf(lp, (1+porto))+".speed")
						if err != nil {
							log.Print("qsfp hget error:", err)
						}
						...
						if strings.Contains(v, "-CR") {
							...
							if media != "copper" {
								ret, err := redis.Hset(redis.DefaultHash, "vnet."+qsfpIfnameOf(lp, (1+porto))+".media", "copper")
								if err != nil || ret != 1 {
							...
							if speed == "100g" {
								ret, err := redis.Hset(redis.DefaultHash, "vnet."+qsfpIfnameOf(lp, (1+porto))+".fec", "cl91")
								if err != nil || ret != 1 {
									log.Print("qsfp hset error:", err, " ", ret)
								} else {
									portConfig += "cl91 "
							...
							qsfpVdev[i+j*16].TxDisableSet(false, lp)
							if media != "fiber" {
								ret, err := redis.Hset(redis.DefaultHash, "vnet."+qsfpIfnameOf(lp, (1+porto))+".media", "fiber")
								if err != nil || ret != 1 {
									log.Print("qsfp hset error:", err, " ", ret)
								} else {
									portConfig += "fiber "
							...
							if speed != "40g" {
								ret, err := redis.Hset(redis.DefaultHash, "vnet."+qsfpIfnameOf(lp, (1+porto))+".speed", "40g")
								if err != nil || ret != 1 {
									log.Print("qsfp hset error:", err, " ", ret)
								} else {
							...
							if speed != "100g" {
								ret, err := redis.Hset(redis.DefaultHash, "vnet."+qsfpIfnameOf(lp, (1+porto))+".speed", "100g")
								if err != nil || ret != 1 {
									log.Print("qsfp hset error:", err, " ", ret)
								} else {
						...
						if !portIsCopper[i+j*16] {
							...
							qsfpVdev[i+j*16].StaticBlocks(i + j*16)
							...
							v = qsfpVdev[i+j*16].TxDisableGet()
	...
	for i := 0; i < 32; i++ {
		...
		if present[i/16]&(1<<uint(i%16)) == 0 {
			if !portIsCopper[i] {
				...
				if qsfpVdev[i].DataReady() {
					qsfpVdev[i].DynamicBlocks(i)
...
func (h *qsfpI2cDev) DataReady() bool {
	var t bool
	r := getRegsLpage0()
...
func (h *qsfpI2cDev) Compliance() string {
	r := getRegsUpage0()
...
func (h *qsfpI2cDev) TxDisableSet(t bool, p int) {
	var b, e bool
...
func (h *qsfpI2cDev) TxDisableGet() string {
	r := getRegsLpage0()
	var v string
...
func (h *qsfpI2cDev) Vendor() string {
	r := getRegsUpage0()
...
func CheckTemp(t uint16, port string) string {
	...
	if update {
		...
		/* FIXME bmc should pull or perhaps subscribe
		if bmcIpv6LinkLocalRedis != "" {
			d, err := redigo.Dial("tcp", bmcIpv6LinkLocalRedis)
			if err != nil {
				log.Print(err)
			} else {
				_, err = d.Do("HSET", redis.DefaultHash+"-bmc", "qsfp.temp.units.C", v)
				if err != nil {
...
func (h *qsfpI2cDev) DynamicBlocks(port int) {
	r := getRegsLpage0()
	rb := getBlocks()
...
func (h *qsfpI2cDev) StaticBlocks(port int) {
